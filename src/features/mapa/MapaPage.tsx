...
export default function MapaPage() {
  ...
  return (
    <div className="-mx-6 -my-6 flex h-[calc(100dvh-96px)] min-h-[560px] flex-col overflow-hidden">
      ...
      {/* ── Barra superior ────────────────────────────────────────────────── */}
      <header className="flex-shrink-0 border-b border-border bg-elevated/95 px-3 py-2.5 sm:px-4">
          ...
          <div className="relative flex min-w-0 flex-1 lg:max-w-md">
            ...
            <input
              ...
              className="h-11 w-full pl-8 pr-3 text-base sm:text-label rounded-lg
                         bg-bg border border-border text-text placeholder:text-muted
          ...
          {(searchResult || searchError) && (
            <button
              ...
              className="flex h-11 w-11 items-center justify-center rounded-lg
                         text-muted hover:text-text border border-border hover:bg-surface transition-all"
            >
              <X size={12} />
            </button>
          )}
        ...
        <button
          ...
          className={`flex min-h-11 items-center gap-2 rounded-lg border px-3 text-caption font-semibold transition-colors
                      ${filtersOpen || activeFilterCount > 0 ? 'border-primary/30 bg-primary/15 text-primary' : 'border-border text-secondary hover:bg-surface'}`}
        >
        ...
        {filtersOpen && (
          <div id="mapa-filtros" className="mt-2 grid grid-cols-2 gap-2 border-t border-border pt-2 sm:grid-cols-3 lg:grid-cols-[repeat(4,minmax(130px,1fr))_auto]">
            ...
            <button type="button" onClick={clearFilters} disabled={activeFilterCount === 0}
                    className="col-span-2 flex min-h-11 items-center justify-center gap-2 rounded-lg border border-border px-3 text-caption font-semibold text-secondary hover:bg-surface disabled:cursor-not-allowed disabled:opacity-40 sm:col-span-1">
      ...
      <div className="relative overflow-hidden overscroll-contain" style={{ flex: '1 1 0', minHeight: 0 }}
        ...
        {showExecucao && execucaoGeo.length === 0 && (
          <div role="status" className="absolute left-2 top-36 z-[500] rounded-lg border border-border bg-elevated/95 px-3 py-2 text-caption text-muted sm:left-4 sm:top-32">
