...
function Conversation({ messages }: { messages: Message[] }) {
  ...
  return <div className="mb-4 max-h-80 space-y-3 overflow-y-auto rounded-xl border border-border bg-bg/40 p-4">{messages.map((message, index) => <div key={index} className={`flex gap-2 ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}><span className={`mt-1 flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full ${message.role === 'user' ? 'order-2 bg-blue/10 text-blue' : 'bg-surface text-secondary'}`}>{message.role === 'user' ? <User size={11} /> : <ChartLineUp size={11} />}</span><div className={`max-w-[85%] rounded-xl border px-3 py-2 text-label leading-relaxed ${message.role === 'user' ? 'border-blue/15 bg-blue/[0.07] text-text' : 'border-border bg-card text-secondary'}`}>{message.role === 'assistant' ? <ReactMarkdown remarkPlugins={[remarkGfm]}>{message.content}</ReactMarkdown> : message.content}</div></div>)}</div>
}
