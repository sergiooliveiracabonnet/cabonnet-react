  ...
  return (
    <div className="space-y-4 animate-fade-in">
        ...
        actions={
          <>
            <details className="relative group">
              ...
              <div className="absolute right-0 top-full mt-2 z-dropdown w-48 rounded-xl border border-strong bg-card-high shadow-xl p-1.5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap border-b border-hairline pb-2">
        ...
        <button
          ...
          className="flex items-center gap-1.5 min-h-9 text-caption font-semibold text-secondary hover:text-text
                     border border-subtle rounded-xl px-3 transition-all duration-fast"
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 text-caption font-semibold
                     ...
                     ${groupBy === 'cliente'
                       ...
                       : 'border-subtle text-secondary hover:text-text'}`}
        >
          <Users size={12} /> Por Cliente
        </button>

        {/* Density toggle */}
        <div className="flex items-center gap-0.5 bg-card border border-subtle rounded-xl p-1">
      ...
      <div className="bg-card border border-subtle rounded-xl p-3 space-y-2">
        ...
        <button onClick={() => setMoreFilters(v => !v)} className={`min-h-9 px-3 inline-flex items-center gap-2 rounded-lg border text-caption font-semibold ${moreFilters ? 'border-primary/40 bg-primary/10 text-primary' : 'border-subtle text-secondary hover:text-text'}`}>
        ...
        {moreFilters && <div className="pt-2 border-t border-hairline flex flex-wrap gap-2 items-center animate-slide-down">
      ...
      {/* Tabela */}
      <div ref={tableRef} className="bg-card border border-subtle rounded-xl overflow-hidden">
        {os.isLoading ? (
        ...
        {!os.equipe && groupBy === 'none' && os.filtered.length > 0 && (
          <div className="flex items-center justify-between px-4 py-3
                          border-t border-hairline text-caption text-muted">
            <div className="flex items-center gap-3">
              ...
              <label className="flex items-center gap-1.5">Por página
                <select value={os.pageSize} onChange={e => { os.setPageSize(Number(e.target.value)); os.setPage(1) }} className="bg-surface border border-subtle rounded-md px-2 py-1 text-text">
