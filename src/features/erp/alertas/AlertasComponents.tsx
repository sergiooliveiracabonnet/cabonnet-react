    ...
    <div className="relative overflow-hidden rounded-2xl border animate-card-enter"
      ...
      <div className="relative flex items-center gap-4 p-4">
        ...
        <button onClick={onToggleAcknowledged} aria-label={acknowledged ? `Marcar ${rule.label} como pendente` : `Reconhecer ${rule.label}`}
          ...
          className={`min-w-10 min-h-10 rounded-lg border flex items-center justify-center hover:text-green hover:border-green/30 focus-visible:ring-2 focus-visible:ring-primary/50 ${acknowledged ? 'border-green/30 bg-green/10 text-green' : 'border-border text-muted'}`}>
          <CheckCircle size={16} />
        </button>
      </div>
    </div>
...
export function SettingsPanel({ settings, onSave, onClose }: {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-stretch justify-end">
      ...
      <div ref={panelRef} role="dialog" aria-modal="true" aria-labelledby="alert-settings-title" className="relative w-full max-w-md bg-elevated border-l border-border
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          ...
          <div className="pt-2 border-t border-border">
            ...
            <div className="space-y-3">
              {rulesD.map(rule => (
                <div key={rule.id} className="rounded-lg border border-border bg-surface/30 p-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <input id={`rule-threshold-${rule.id}`} type="number" value={rule.threshold}
                        ...
                        className="w-16 h-9 rounded-md border border-border bg-card px-2 text-center text-label font-mono text-text outline-none focus-visible:ring-2 focus-visible:ring-primary/50" />
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="p-5 border-t border-border flex gap-2">
          <button onClick={onClose}
            className="flex-1 py-2.5 rounded-xl border border-border text-label text-secondary
