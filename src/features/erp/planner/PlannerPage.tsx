  ...
  return (
    <div className="space-y-4 max-w-[1600px]">
        ...
        {[
          ...
          <div key={i}
            ...
            <div className="p-4">
              ...
              <p className="font-mono font-black tabular-nums text-[28px] leading-none" style={{ color: k.color }}>{k.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl border border-white/[0.08] bg-card p-4">
        <p className="text-[10px] font-bold uppercase tracking-[0.06em] text-muted mb-3">Carga total por dia</p>
          ...
          {loadDays.map(d => {
            ...
            return (
              <div key={d.key} className="flex-1 flex flex-col items-center">
                ...
                <span className={`text-[9px] font-bold mt-1 ${d.isToday ? 'text-primary' : 'text-muted'}`}>{d.dow}</span>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/[0.08] bg-surface/30">
                    <th className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-[0.05em] text-muted w-[160px]">
                      Equipe
                    </th>
                    {days.map(d => (
                      <th key={d.key}
                          className={`px-2 py-3 text-center text-[10px] font-bold border-r border-white/[0.04] last:border-r-0 w-[100px]
                                      ${d.isToday ? 'text-primary' : d.isWeekend ? 'text-muted/70' : 'text-muted'}
                                      ${d.isWeekend ? 'bg-surface/10' : ''}`}>
                        <div>{d.dow}</div>
                        <div className={`text-[9px] font-normal mt-0.5 ${d.isToday ? 'text-primary/70' : ''}`}>{d.label}</div>
                        {d.isToday && <div className="w-1 h-1 rounded-full bg-primary mx-auto mt-0.5" />}
                      </th>
                    ))}
                    {editMeta && (
                      <th className="px-3 py-3 text-center text-[10px] font-bold uppercase tracking-[0.05em] text-muted w-[70px]">
                        Meta/sem
                      </th>
                    )}
                    <th className="px-3 py-3 text-right text-[10px] font-bold uppercase tracking-[0.05em] text-muted w-[80px]">
                      Total
                    </th>
                  </tr>
                </thead>
