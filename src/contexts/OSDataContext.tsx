...
const EMPTY_DERIVED = {
  dashboard: {
    ...
    pulso: {
      ...
      agingMed: 0, agingDist: { ok: 0, limite: 0, estourado: 0, critico: 0 } as { ok: number; limite: number; estourado: number; critico: number },
      slaFila: 0, taxa: 0, slaAtingimento: null as number | null, semAgendamento: 0,
      mttr: 0, mttrP90: 0, backlogDias: null as number | null,
      ...
      metaMes: {
        ...
        frentes: 0, prodFrenteDia: 0,
      },
      ritmoIntradiario: {
        manha: 0, tarde: 0, semPeriodo: 0, tardeIniciada: false,
        fracTarde: 0, esperadoTarde: 0, alerta: false,
      },
    },
    mudancas: [] as { id: string; label: string; atual: number; anterior: number; delta: number; unidade: string; melhorou: boolean; variacao: number }[],
  ...
  churn: {
    janelaDias: 60,
    clientes: [] as { chave: string; cliente: string; cidade: string; bairro: string; visitas: number; intervaloMedio: number; diasDesdeUltima: number; rows: OSRow[] }[],
    totalReincidentes: 0,
    totalBase: 0,
    pctReincidencia: 0,
  },
...
export function OSDataProvider({ children }: { children: ReactNode }) {
  ...
  const builderErrors = useMemo(() => [
    ...
    churn     === EMPTY_DERIVED.churn     && 'churn',
