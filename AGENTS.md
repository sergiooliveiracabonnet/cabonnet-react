...
The `_operadora_da_os(row)` function in `cabonnet/telegram.py` drives all operator filtering. Adding a new operator means updating `_OPERADORA_GRUPOS`, `_operadora_for_chat`, `_label_operadora`, `_tg_broadcast_status_changes`, `_enviar_executadas`, `_grupo_cmds`, and the `_CHAT_MAP` in notify endpoints.
