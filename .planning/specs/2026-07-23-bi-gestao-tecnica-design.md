...
- **Achado na implementação:** o gráfico "Total de OS por Mês" acabou usando `/backlog` (escopado ao período selecionado) como fonte única para as 4 abas, em vez de `allRows`/`OSDataContext` como esta spec previa originalmente para esse gráfico específico. Consequência: no preset padrão "Mês Atual", o gráfico mostra só 1 mês — a série multi-mês só aparece se o usuário escolher manualmente um período "Personalizado" largo. Decisão (2026-07-23, revisão final): manter assim por ora — é simplicidade consciente (fonte única), não bug. Revisitar se o histórico multi-mês por padrão fizer falta na prática.
